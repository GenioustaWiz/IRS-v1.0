...
def Search():
    if SEARCH.get() != "":
        ...
        connection()
        cursor.execute("SELECT * FROM `usr` WHERE `loginname` LIKE ?", ('%'+str(SEARCH.get())+'%',))
        fetch = cursor.fetchall()
        for data_ in fetch:
            #data = (data[1],data[2],data[3],data[4],data[5],data[6],data[6]) # gettting only the required data from Database
            #print(data)
            tree.insert('', 'end', values=(data_))
