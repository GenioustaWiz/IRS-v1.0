...
def LoginForm():
    if alert == 1:
        ...
        username.bind("<Button-1>", clear_usrname) #bind the mouse button, when clicked words disappear
        password = Entry(MidLoginForm, textvariable=PASSWORD, font=('arial', 25), width=15, show= "p")
        password.grid(row=1, column=1,pady=30)
        password.insert(0,"Password")
        password.bind("<Button-1>", clear_usrpass) #bind the mouse button, when clicked words disappear
        btn_login = Button(MidLoginForm, text="Login", font=('arial', 18), width=30, command=Login, fg="green")
        btn_login.grid(row=2, columnspan=2, pady=20)
        btn_login.bind('<Return>', Login)

def clear_usrname(Event):
    USERNAME.set("")  
def clear_usrpass(Event):
    PASSWORD.set("") 
...
def menu3(root):
    ...
    filemenu.add_command(label="Admin Login", command=LoginForm)
    filemenu.add_command(label="Logout", command=Logout)
    filemenu.add_command(label="Exit", command=Exit)
